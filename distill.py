...
data_list = [
    ...
    "pensum_sistemas",
] # total 1200 preguntas

model_name="google/gemma-3-12b-it"
quantization_config = None
torch_dtype = torch.bfloat16
tokenizer_max_length = 256000 # <-- Aumentar si es necesario, pero cuidado con VRAM/tiempo
max_new_tokens = 150 # Quizás aumentar un poco si las respuestas necesitan ser más largas
output_dir = "kd_data_gemma3_12b_it" 
...
TEACHER_VOCAB_SIZE_FROM_TOKENIZER = len(tokenizer)
print(f"INFO: Tamaño del vocabulario del profesor (obtenido de len(tokenizer)): {TEACHER_VOCAB_SIZE_FROM_TOKENIZER}")
print(f"INFO: Tamaño del vocabulario del profesor (tokenizer.vocab_size): {tokenizer.vocab_size}")
...
with open(output_jsonl_path, "w", encoding="utf-8") as f_out:
    ...
    for data_name in data_list:
        print(f"Procesando conjunto -> : {data_name}")
        questions_w_context = load_prompts_with_context(data_name)
        ...
        for prompt_info in questions_w_context["questions"]:
            ...
            **Contexto del Documento Completo:**
            ...
            # Decodifica la respuesta generada por el profesor
            raw_decoded_text = tokenizer.decode(target_token_ids, skip_special_tokens=True)

            # Define los tokens/secuencias no deseadas a eliminar
            unwanted_sequences = [
                "<|eot_id|>",
            ...
            cleaned_text = raw_decoded_text
            for seq in unwanted_sequences:
                cleaned_text = cleaned_text.replace(seq, "")

            # Elimina espacios en blanco al principio/final que puedan quedar tras los reemplazos
            ...
            metadata = {
                ...
                "respuesta_generada_profesor_texto": cleaned_text,
